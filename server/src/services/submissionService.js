...
export async function completeChallenge(studentId, challengeId) {
  ...
  if (existingSubmission) {
    const error = new Error("Challenge already completed by this student.");
    error.status = 409;
    throw error;
  }

  await db.run("INSERT INTO submissions (student_id, challenge_id) VALUES (?, ?)", [studentId, challengeId]);
  ...
  return { student: updatedStudent, challenge, unlockedBadges };
}
